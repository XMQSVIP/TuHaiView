...
> 历史记录归档。下文“当前”“最终”等称谓均对应当时的候选，不代表现在的交付状态。最新 EXE 与验收结果以 [PERFORMANCE.md](PERFORMANCE.md) 为准；保留这些原始叙述以便核对测量演变。

## 当前状态：原生网格上传复用候选

06:23 后固定候选为 `4507D93F1BD9C484065FA3710E1C89C59B7E7E45A7631ED4526A44597C1225EA`（源码 `a504605`），包含下面上传改动以及输入模态计时修正。40 项 Release 回归与两个 GPU 读回回归通过；后续测试全部按新哈希归档。下文 `FAE039A3…` 的单轮内存通过只属于前一个计时版本。

显示环境追加证据：活动目标标志 `friendlyNameForced=1`，Windows 报告未检测到连接显示器并强制启用目标，目标信号仍报约 60 Hz。独立五轮 DXGI 垂直空白等待也约 32 ms。[显示路径诊断](performance-results/20260906/DISPLAY-PATH.md) 保存原始数据与官方语义。不能由此归因到特定驱动或将失败改为通过；实际物理显示器体验仍缺验证环境。

新增候选 `FAE039A3…` 将 egui 顶点／索引的反复临时上传分配改为原生 StagingBelt 复用。独立控件渲染探针的 private bytes 变化由约 +9.12 MiB／分钟降至 −0.05 MiB／分钟；32 帧 GPU 逐像素对照及丢弃未提交编码器回归通过，产品 40 项 Release 回归和图片 GPU 读回通过。方法、原始结果和限制见 [上传内存诊断](performance-results/20260906/MESH-UPLOAD.md)。短探针不能代替完整应用长时间验收。

下文 `19199B97…` 的 80 轮短矩阵是上一候选的完整对照数据。新 EXE 的受影响场景需要重新运行；目前仍未达到全部发布条件，默认呈现模式和图片资源预算保持原值。

该上传候选的 600 秒 HDD 操作循环正常结束，日志和显示采集有效；第 4／8 分钟 private bytes 中位数相差 +3.08 MiB，末段待处理和回收项归零，管理资源预算通过。显示 P95 仍为 32.034 ms，未达标。随后同候选五轮原生基准：数据库中位数 3.12 s／同轮旧路径 3.47 s；24/48 MP JPEG 解码减少 77.24%／78.73%；1,000 张真实 JPEG 缓存减少 93.64%。逐轮和哈希见 `mesh-reuse-native-summary.json`。这些不能替代最终 EXE 的十轮长测。

06:18 该候选首轮完整 30 分钟 SSD 内存验收通过：斜率 **0.1005 MiB／分钟**，末段增长 **3.9414 MiB**，所有待处理／回收指标归零，四项管理预算通过。它是一个有效单轮，尚非最终 EXE 的 SSD/HDD 各五轮完成记录。详见 `mesh-reuse-30min-ssd.json`。

人工输入检查发现原生文件夹对话框的等待混入输入墙钟时间，已在后续源码中增加单独标记和扣除后的处理指标，原始总时长仍保留。工具缺少可用截图／鼠标坐标及输入到显示样本，输入验收尚未通过，详见 [输入验证边界](performance-results/20260906/INPUT-ATTEMPT.md)。
...
同一 EXE 的合成 10k/50k、SSD/HDD 静止打开与固定路线滚动已各完成五轮。10k SSD/HDD 首批记录中位数分别为 **345.76 / 354.91 ms**；50k HDD 为 **478.60 ms**。这些组的滚动仍未满足 60 Hz P95 目标。所有逐轮显示中位数、P95、P99、最大值和 >50/>100 ms 次数见 [短场景逐轮报告](performance-results/20260906/SHORT-MATRIX.md)，不以跨轮平均值覆盖某轮失败。

截至 04:57，同一 EXE 的 **8 个数据/磁盘组合 × 打开/滚动 × 5 轮，共 80 次正式短场景全部有效**。组合包含合成 10k/50k 的 SSD/HDD、真实 10k/50k HDD、相同 1,333 张真实图的 SSD/HDD。所有应用管理资源预算通过；40 轮缓存滚动均可见纹理全命中，均无 >50/>100 ms 停顿，最大显示间隔 **32.80 ms**。但是 P95 为 **31.868～32.072 ms**，**40 轮均未达到 60 Hz 的 17.167 ms P95 上限**。真实 10k HDD 首批记录中位数 349.92 ms，首屏资源就绪 388.12 ms。

报告校验新增请求时长、期望目录数量、日志与进程/场景关联、独立运行 ID 和实际窗口尺寸比较；无效轮次不参与跨轮性能统计。30 分钟内存判定要求完整时长及稳态每分钟至少 50 个样本。当前报告工具 **12 项测试通过**；这不增加产品功能测试或未执行验收的通过数。
