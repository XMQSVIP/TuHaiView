...
追加验证 EXE：`19199B97FB4E929D4495DDD839ED2D6BAD7C50C44AD9B066AF1C7ED75A845B69`，源代码 `433cebc`，默认 features，17,370,112 字节。Release 回归 **40 通过、0 失败、5 项手动测试默认忽略**；手动 GPU 读回/取消也通过。工作区 `target/release/TuHaiView.exe` 已更新为此验证版。

该 EXE 在 F 盘真实 50,000 张缓存集完成五轮静止打开，全部日志和显示采集有效、全部保持顶部。从 `main` 到首批记录中位数 **517.71 ms**、最慢 **563.03 ms**；到首屏资源就绪中位数 **547.24 ms**、最慢 **585.97 ms**。这包含窗口/GPU 初始化，但不包括进入 `main` 前的 OS 加载，首屏资源就绪也不是输入到光子的测量。F/G 同属 HDD，系统文件缓存未知；不能代替 SSD 启动与系统冷缓存验收。完整数据见 `real-50k-cached-open.json` 与 F 盘逐轮原始记录。

同一真实集五轮缓存滚动全部采样有效且可见项全命中，显示 P95 分别 **32.016、32.052、32.067、32.050、32.048 ms**，P99 为 32.136～32.174 ms，均无超过 50/100 ms 的停顿。P95 全部超过 60 Hz 条件的 17.167 ms 上限，**五轮均未通过滚动目标**。这是一组有效的失败测量，不是缺样推断。逐轮统计与预算峰值见 `performance-results/20260906/real-50k-cached-scroll.json`。

SSD 上相同 EXE、合成 50,000 张已缓存集的五轮静止打开均有效：从 `main` 到首批记录中位数 **399.14 ms**、最大 **423.87 ms**；首屏资源就绪中位数 **431.61 ms**、最大 **459.41 ms**。首批资源就绪达到 1 秒目标，仍单独保留进入 `main` 前加载和实际呈现的测量边界。详见 `synthetic-50k-ssd-open.json`。
