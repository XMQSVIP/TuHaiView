...
当前源码已在下方 v5 性能基线之后加入文件名搜索、界面打磨和扫描顺序稳定修复。最新产物、哈希及功能回归见 [扫描顺序修复报告](performance-results/20260906/SCAN-ORDER.md)。该版 Release 常规测试 56 项通过，另行执行的 DX12 全应用回归通过；50k 搜索应用耗时 P95 为 169.93～189.35 ms（含防抖和测试帧泵），不代表原生屏幕呈现延迟。

本次没有重跑完整 PresentMon 对照或长期内存矩阵。下面 v5 的提交、EXE/ZIP 哈希、版本字段及测量保留为历史证据，不能当作当前修复版的交付信息或完整性能验收结论。
