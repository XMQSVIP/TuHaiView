#![allow(unsafe_code)]
// Modified for TuHaiView: reuse mesh upload buffers on the native winit path.
...
impl Renderer {
    ...
    pub fn update_buffers(
        ...
        if index_count > 0 {
            ...
            let size = NonZeroU64::new(required_index_buffer_size).unwrap();
            if let Some(belt) = self.staging_belt.as_mut() {
                let mut mapped =
                    belt.write_buffer(encoder, &self.index_buffer.buffer, 0, size, device);
                copy_mesh_data(
                    paint_jobs,
                    &mut mapped,
                    &mut self.index_buffer.slices,
                    |mesh| bytemuck::cast_slice(&mesh.indices),
                );
            } else {
                let mut mapped = queue
                    .write_buffer_with(&self.index_buffer.buffer, 0, size)
        ...
        if vertex_count > 0 {
            ...
            let size = NonZeroU64::new(required_vertex_buffer_size).unwrap();
            if let Some(belt) = self.staging_belt.as_mut() {
                let mut mapped =
                    belt.write_buffer(encoder, &self.vertex_buffer.buffer, 0, size, device);
                copy_mesh_data(
                    paint_jobs,
                    &mut mapped,
                    &mut self.vertex_buffer.slices,
                    |mesh| bytemuck::cast_slice(&mesh.vertices),
                );
            } else {
                let mut mapped = queue
                    .write_buffer_with(&self.vertex_buffer.buffer, 0, size)
                    .expect("vertex staging allocation");
                copy_mesh_data(
                    paint_jobs,
                    &mut mapped,
                    &mut self.vertex_buffer.slices,
    ...
    /// Previous native frame was submitted, or its encoder was dropped on a surface error.
    pub(crate) fn begin_native_mesh_uploads(&mut self) {
        self.staging_belt
            .get_or_insert_with(|| wgpu::util::StagingBelt::new(256 * 1024))
            .recall();
    }
}
...
fn copy_mesh_data(
    paint_jobs: &[epaint::ClippedPrimitive],
    destination: &mut [u8],
    slices: &mut Vec<Range<usize>>,
    data: fn(&epaint::Mesh) -> &[u8],
) {
    let mut offset = 0;
...
mod tuhai_tests {
    ...
    #[test]
    #[ignore = "manual DX12 upload reuse/readback and abandoned encoder regression"]
    fn native_mesh_reuse_matches_queue_upload_after_abandoned_frames() {
        let instance = wgpu::Instance::new(&wgpu::InstanceDescriptor {
            backends: wgpu::Backends::DX12,
            ..Default::default()
        });
        let state = pollster::block_on(crate::RenderState::create(
            &crate::WgpuConfiguration::default(),
            &instance,
            None,
            None,
            1,
        ...
        let target = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("mesh reuse regression"),
            size: wgpu::Extent3d {
                width: 64,
                height: 64,
                depth_or_array_layers: 1,
        ...
        let mut baseline = Renderer::new(device, wgpu::TextureFormat::Rgba8Unorm, None, 1, false);
        let mut pooled = Renderer::new(device, wgpu::TextureFormat::Rgba8Unorm, None, 1, false);
        for renderer in [&mut baseline, &mut pooled] {
            renderer.update_texture(
                device,
                queue,
                epaint::TextureId::Managed(0),
                &epaint::ImageDelta::full(
                    epaint::ColorImage::new([1, 1], epaint::Color32::WHITE),
        ...
        for frame in 0..32 {
            ...
            if frame % 4 == 0 {
                ...
                pooled.update_buffers(device, queue, &mut abandoned, &jobs, &screen);
                drop(abandoned); // equivalent to a surface-error frame with no submission
            }
            let mut outputs = Vec::new();
            for (reuse, renderer) in [(false, &mut baseline), (true, &mut pooled)] {
                if reuse {
                ...
                outputs.push(readback.slice(..).get_mapped_range().to_vec());
                readback.unmap();
            }
            assert!(outputs[0].iter().any(|b| *b != 0));
            assert_eq!(
                outputs[0], outputs[1],
                "frame {frame}: mesh changed after reuse"
            );
        }
    }
}
